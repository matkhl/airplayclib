...
int32_t ALACDecoder::Init( void * inMagicCookie, uint32_t inMagicCookieSize )
{
    ...
    if (theCookieBytesRemaining >= sizeof(ALACSpecificConfig))
    {
        ...
        // Defense-in-depth: reject an absurd frameLength so a misparsed magic
        // cookie cannot request multi-GiB calloc buffers below (each buffer is
        // frameLength*4 bytes). Legitimate AirPlay frame lengths are <= 16384.
        RequireAction( mConfig.frameLength != 0 && mConfig.frameLength <= 65536,
